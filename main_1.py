...
#   {checkpoint_path}/runs/{timestamp}_j{JOB}[_a{ARRAY}]_{subject}/
...
def prepare_run_dirs(args, subject, checkpoint_root):
    ...
    Layout:
      {checkpoint_root}/runs/{YYYYMMDD-HHMMSS}_j{JOB}_a{ARRAY}_{subject}/
    ...
    array_id = os.environ.get('SLURM_ARRAY_TASK_ID', '').strip()
    if array_id:
        run_name = f'{ts}_j{job_id}_a{array_id}_{subject}'
    else:
        run_name = f'{ts}_j{job_id}_{subject}'
